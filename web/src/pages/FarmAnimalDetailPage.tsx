...
import {
  ...
  deleteFarmVaccine, updateFarmAnimal, archiveFarmAnimal,
...
export function FarmAnimalDetailPage() {
  ...
  async function handleArchive() {
    if (!user || !animalId || !animal) return
    ...
    try {
      await archiveFarmAnimal(user.uid, animalId)
      navigate('/farm')
    } catch {
      ...
      <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 sticky top-0 z-10">
        <div className="flex items-center gap-2 px-4 pt-safe h-14">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-base font-semibold text-gray-900 dark:text-white truncate">{pageTitle}</p>
            {animal && (
        ...
        <div className="px-4 py-4 pb-32 flex flex-col gap-4">
          {animal && (
            <>
              ...
              {/* ── Danger zone ── */}
              <div className="bg-white dark:bg-gray-800 rounded-xl border border-red-200 dark:border-red-900/40 overflow-hidden">
                <div className="bg-red-50 dark:bg-red-900/20 px-4 py-2.5 border-b border-red-200 dark:border-red-900/40">
                  <span className="text-[10px] font-bold text-red-500 dark:text-red-400 uppercase tracking-widest">Danger Zone</span>
                </div>
                <div className="px-4 py-4 flex items-center justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-gray-900 dark:text-white">Archive Animal</p>
                    <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">
                      Remove from active herd. Record is preserved and can be restored from the archive.
                    </p>
                  </div>
                  <button
                    onClick={handleArchive}
                    className="flex-shrink-0 flex items-center gap-1.5 px-4 py-2.5 rounded-lg border border-red-300 dark:border-red-700 text-xs font-semibold text-red-600 dark:text-red-400 active:bg-red-50 dark:active:bg-red-900/20"
                  >
                    <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
